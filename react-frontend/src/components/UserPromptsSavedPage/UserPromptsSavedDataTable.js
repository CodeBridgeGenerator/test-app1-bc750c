...
const UserPromptsSavedDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    ...
    const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.chatAiId}</p>
    const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.savedUserid}</p>
    ...
    return (
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorLeft={paginatorLeft} paginatorRight={paginatorRight} rowClassName="cursor-pointer">
            <Column field="chatAiId" header="Chat Ai Id" body={pTemplate0} style={{ minWidth: "8rem" }} />
            <Column field="savedUserid" header="Saved Userid" body={pTemplate1} style={{ minWidth: "8rem" }} />
