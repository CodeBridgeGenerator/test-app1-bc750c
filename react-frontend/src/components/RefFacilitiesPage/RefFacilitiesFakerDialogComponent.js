...
import refFacilitiesFakerFactory from "./refFacilitiesFakerFactory";
...
const RefFacilitiesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = refFacilitiesFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("refFacilities").create(elem));
...
export default connect(mapState, mapDispatch)(RefFacilitiesFakerDialogComponent);
