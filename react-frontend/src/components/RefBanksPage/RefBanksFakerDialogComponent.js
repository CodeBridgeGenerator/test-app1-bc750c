...
import refBanksFakerFactory from "./refBanksFakerFactory";
...
const RefBanksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = refBanksFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("refBanks").create(elem));
...
export default connect(mapState, mapDispatch)(RefBanksFakerDialogComponent);
